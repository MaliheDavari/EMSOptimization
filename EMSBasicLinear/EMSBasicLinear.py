...
cost_const=m.addConstr(cost == quicksum(P_grid[t] * TI * price_utility[t] for t in T) +
                         quicksum(P_gen[t, g] * TI * price_p_gen[g] + price_c_gen[g] * TI for t in T for g in G),
                         name='cost_function')

P_balance_const=m.addConstrs((P_load[t]==P_grid[t]+ quicksum(P_gen[t,g] for g in G)+
...
plt.figure()
plt.plot(T,[P_res[t,'pv'].x for t in T],label="PV")
plt.plot(T,[P_res[t,'wt'].x for t in T],label='Wind Turbine')
plt.plot(T, [P_grid[t].x for t in T], label='Power from Grid')
plt.plot(T, [P_gen[t,'gen1'].x for t in T], label='Generator number 1')
plt.plot(T,[P_gen[t,'gen2'].x for t in T], label='Generator number 2')
plt.plot(T,[P_load[t] for t in T], label='Load')
